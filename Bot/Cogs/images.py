...
class ImageScraperV2(commands.Cog):
    ...
    @slash_command(
        name="image",
        description="Scrapes some images off of DeviantArt",
        guild_ids=[866199405090308116],
    )
    async def imageScraperV2(
        self, ctx, *, search: Option(str, "The search query to perform")
    ):
        async with aiohttp.ClientSession(json_serialize=orjson.dumps) as session:
            params = {"q": search}
            async with session.get(
                "https://www.deviantart.com/search", params=params
            ) as r:
                data = await r.text()
                ...
                embedVar.set_image(url=random.choice(links))  # nosec B311
