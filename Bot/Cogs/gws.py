...
from genshin_wish_sim_utils import (MikuWSUserInvUtils, MikuWSUsersUtils,
                                    MikuWSUtils)
...
wsUtils = MikuWSUtils()
wsUserInvUtils = MikuWSUserInvUtils()
wsUserUtils = MikuWSUsersUtils()
...
class MikuGWSBanners(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    gws = SlashCommandGroup("gws", "Commands for Miku's Genshin Wish Sim")
    ...
    async def purgeUserInv(self, ctx):
        ...
        await ctx.respond(embed=embed, view=PurgeView(), empheral=True)
...
def setup(bot):
    bot.add_cog(MikuGWSBanners(bot))
